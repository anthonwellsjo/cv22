...
const Skills: React.FC<props> = ({ tech, techTypes }) => {
  ...
  return (
    <div style={{position: "relative", display: "flex", alignItems: "center", width: "90%",marginTop:isDesktop?"5%": "15%", height: "100%", zIndex: 1, flexDirection:"column",  }}>
      <div style={{ display:"flex",height:"20%", width: "70%", maxWidth: "400px",  justifyContent: "center",flexDirection:"row" }}>
        {techTypes.map(t => {
          return (
            <h4 onClick={() => { onClickCatEvent(t) }} key={t} style={{ margin: 0, padding: 0, cursor: "pointer", fontFamily: "Text", fontWeight: categoryCurrentlyViewed === t ? "bold" : "normal"}} >{t}</ h4>
          )
        })}
      </div>
      <div style={{ display:"flex",height:"20%", width: "60%",justifyContent: "center",flexDirection:"row", }}>
        {visibleTech && <TechContainer itemSize={getTechItemSize(width!)} tech={visibleTech.length === 0 ? tech : visibleTech} itemsPerRow={getTechItemsPerRow(width!)} />}
      </div>
    </div >
  )
